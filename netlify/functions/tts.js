/**
 * Server-side proxy to the code-mixed Bangla TTS Space.
 *
...
/** "Ataullha/Codemix_BanglaTTS_Public_Demo" -> the hf.space host */
...
exports.handler = async (event) => {
  ...
  let text = '';
  let enableCodemix = true;
  try {
    const body = JSON.parse(event.body || '{}');
    text = String(body.text || '').trim();
    if (typeof body.enable_codemix === 'boolean') enableCodemix = body.enable_codemix;
  } catch {
  ...
  for (const fn of FUNCTIONS) {
    const attempt = await callSpace(host, fn, [text, enableCodemix]);
    if (attempt.result) {
  ...
  const normalised =
    Array.isArray(result) && typeof result[0] === 'string' && result[0].trim() ? result[0] : null;
